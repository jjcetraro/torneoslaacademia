...
export default function TournamentBracketsComponent({
  tournament,
  permitEdition,
}) {
    ...
    if (targetName.startsWith("player1")) {
      if (targetName.endsWith("set1")) {
        setModalData({
          ...
          result: [
            ...
            [
              modalData.result[1][0],
              modalData.result[1][1],
              modalData.result[1][2],
            ],
          ],
        });
      ...
      if (targetName.endsWith("set1")) {
        setModalData({
          ...
          result: [
            [
              modalData.result[0][0],
              modalData.result[0][1],
              modalData.result[0][2],
            ],
  ...
  const handleClickLoadResult = async (event) => {
    ...
    const matchToUpdate = new Match(
      ...
      new Player(
        document.getElementById("selectPlayer1").value !== "0"
          ? document.getElementById("selectPlayer1").value
          : null,
        null,
        null,
        null,
      ...
      new Player(
        document.getElementById("selectPlayer2").value !== "0"
          ? document.getElementById("selectPlayer2").value
          : null,
        null,
        null,
        null,
      ...
      if (phase) {
        ...
        if (lastMatch) {
          if (className.includes("bb")) {
            ...
            player = (
              <div style={{ paddingTop: "10px" }}>
                <BracketMatchLine
                  ...
                  set1={
                    lastMatch.getResult()
                      ? lastMatch.getResult().player1[0]
                      : "-"
                  }
                  set2={
                    lastMatch.getResult()
                      ? lastMatch.getResult().player1[1]
                      : "-"
                  }
                  set3={
                    lastMatch.getResult()
                      ? lastMatch.getResult().player1[2]
                      : "-"
                  }
                  handleClick={
                    permitEdition ? () => handleClick(lastMatch) : () => {}
                  }
                />
              </div>
            );
            ...
            i > 0 &&
            tableRows[i - 1].props.children[j].props.className.includes("bb")
          ) {
            ...
            player = (
              <div>
                <BracketMatchLine
                  ...
                  set1={
                    lastMatch.getResult()
                      ? lastMatch.getResult().player2[0]
                      : "-"
                  }
                  set2={
                    lastMatch.getResult()
                      ? lastMatch.getResult().player2[1]
                      : "-"
                  }
                  set3={
                    lastMatch.getResult()
                      ? lastMatch.getResult().player2[2]
                      : "-"
                  }
                  handleClick={
                    permitEdition ? () => handleClick(lastMatch) : () => {}
      ...
      const td = (
        <td
          ...
          style={{
            ...
            verticalAlign: isPlayer2 ? "top" : "middle",
          }}
        >
          {player}
        </td>
  ...
  return (
    <>
      ...
      {!lastMatchClicked ? (
        ...
        <div>
          <div closeButton>
            <div>Cargar Resultado</div>
          </div>
          <div>
            <div style={{ display: "flex" }}>
              <span>Fue W.O.:</span>
              <select id="selectWO" style={{ flexGrow: "1" }}>
          ...
          <div style={{ display: "block", textAlign: "center" }}>
            <div className="p-5 bg-blue-100" onClick={handleClickLoadResult}>
              Cargar
            </div>
          </div>
        </div>
...
function getClassName(row, col, phases) {
  ...
  if (
    col < phases - 1 &&
    row % Math.pow(2, col + 2) >= Math.pow(2, col) &&
    row % Math.pow(2, col + 2) <= Math.pow(2, col + 2) - Math.pow(2, col) - 1
  ) {
    result += " br";
  }
